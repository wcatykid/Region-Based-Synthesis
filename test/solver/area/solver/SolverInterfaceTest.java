...
public class SolverInterfaceTest
{
    ...
    public void test()
    {
        ...
        runProblemArea(1, "{ 2 - x^2 ; x }                           <4.5>       // LH, Page 379 Example 2; Implied Domain;");
        runProblemArea(1, "{ 2 - x^2 ; x }                  [-2, 1]  <4.5>       // LH, Page 379 Example 2; Repeated but with explicit domain;");
        ...
        runProblemArea(1, "{ 2 ; Sec[x] }              [-PI/3,PI/3]  <1.5548744> // LH, Page 384 #28;");
        runProblemArea(1, "{ 2*Sin[x] ; Tan[ x ] }       [-1.5,1.5]  <2.807927>  // LH, Page 384 #29;");
        runProblemArea(1, "{ Sin[2x] ; Cos[x] }          [-1.5,1.5]  <2.489992>  // LH, Page 384 #30;");
        runProblemArea(1, "{ 2*Sin[x] + Sin[2*x] ; 0 }      [0,3.5]  <3.995962>  // LH, Page 384 #31;");
        runProblemArea(1, "{ 2*Sin[x] + Cos[2*x] ; 0 }      [0,3.5]  <4.2014066> // LH, Page 384 #32;");
        ...
        runProblemArea(1, "{ Csc[x] ; 2 }              [PI/6,5*PI/6] <1.5548744> // LH, Page 431 #16;");
        runProblemArea(1, "{ Sin[x] ; Cos[x] }      [.25*PI,1.25*PI] <2.8284271> // LH, Page 431 #17;");
        runProblemArea(1, "{ Cos[x] ; .5 }             [PI/3,7*PI/3] <4.5112991> // LH, Page 431 #18;");
        runProblemArea(1, "{ 5x-x^2 ; x }                            <10.666666> // Stewart, Page 352 #1; Implied Domain;");
        runProblemArea(1, "{ Surd[x+2,2] ; 1/(x+1) }        [0,2]    <2.3491029> // Stewart, Page 352 #2;");
        runProblemArea(1, "{ x^2-1 ; Surd[x,2] }            [0,1]    <1.3333333> // Stewart, Page 352 #3;");
        runProblemArea(1, "{ x^2-4x ; 2x-x^2 }                       <9>         // Stewart, Page 352 #4; Implied Domain;");
        runProblemArea(1, "{ x+1 ; 9-x^2 }                  [-1,2]   <19.5>      // Stewart, Page 352 #5;");
        runProblemArea(1, "{ Sin[x] ; x }                  [PI/2,PI] <2.7010912> // Stewart, Page 352 #6;");
        runProblemArea(1, "{ x ; x^2 }                               <0.1666666> // Stewart, Page 352 #7; Implied Domain;");
        runProblemArea(1, "{ x^2-2x ; x+4 }                          <20.833333> // Stewart, Page 352 #8; Implied Domain;");
        runProblemArea(1, "{ Surd[x+3,2] ; (x+3)/2 }                 <1.3333333> // Stewart, Page 352 #9; Implied Domain;");
        runProblemArea(1, "{ 1+Surd[x,2] ; (3+x)/3 }                 <4.5>       // Stewart, Page 352 #10; Implied Domain;");
        runProblemArea(1, "{ x^2 ; Surd[x,2] }                       <0.3333333> // Stewart, Page 352 #11; Implied Domain;");
        runProblemArea(1, "{ x^2 ; 4x-x^2 }                          <2.6666666> // Stewart, Page 352 #12; Implied Domain;");
        runProblemArea(1, "{ 12-x^2 ; x^2 - 6 }                      <72>        // Stewart, Page 352 #13; Implied Domain;");
        runProblemArea(1, "{ Cos[x] ; 2-Cos[x] }            [0,PI*2] <12.566370> // Stewart, Page 352 #14;");
        runProblemArea(1, "{ Sec[x]^2 ; 8*Cos[x] }      [-PI/3,PI/3] <10.392304> // Stewart, Page 352 #15;");
        runProblemArea(1, "{ x^3-x ; 3x }                            <8>         // Stewart, Page 352 #16; Implied Domain;");
        runProblemArea(1, "{ Surd[x,2] ; .5x }              [0,9]    <4.9166666> // Stewart, Page 352 #17;");
        runProblemArea(1, "{ 8-x^2 ; x^2 }                  [-3,3]   <30.666666> // Stewart, Page 352 #18;");
        runProblemArea(1, "{ 2x^2 ; 4+x^2 }                          <10.666666> // Stewart, Page 352 #19; Implied Domain;");
        runProblemArea(1, "{ .25*(12 - x^2) ; x }                    <21.333333> // Stewart, Page 352 #20; Implied Domain;");
        runProblemArea(1, "{ 1-x^2 ; x^2-1 }                         <2.6666666> // Stewart, Page 352 #21; Implied Domain;");
        runProblemArea(1, "{ Sin[1.57079633*x] ; x }                 <0.2732395> // Stewart, Page 352 #22; Implied Domain; The original problem had PI/2, rather than 1.57079633, but Mathematica wouldn't answer that;");
        runProblemArea(1, "{ Cos[x] ; Sin[2x] }             [0,PI/2] <0.4999999> // Stewart, Page 352 #23;");
        runProblemArea(1, "{ Cos[x] ; 1 - Cos[x] }          [0,PI]   <4.5112912> // Stewart, Page 352 #24;");
        runProblemArea(1, "{ Cos[x] ; 1-0.63661977*x }               <0.4292036> // Stewart, Page 352 #25; Implied Domain; The original problem had 2/PI, rather than 0.63661977, but Mathematica wouldn't answer that;");
        runProblemArea(1, "{ Abs[x] + 2 ; x^2 }                      <6.6666666> // Stewart, Page 352 #26; Implied Domain;");
        runProblemArea(1, "{ 1/x^2 ; x ; x/8 }                       <0.7499999> // Stewart, Page 352 #27; Implied Domain;");
        runProblemArea(1, "{ 3x^2 ; 8x^2 ; 4-4x }           [0,1]    <1.6666666> // Stewart, Page 352 #28;");
        runProblemArea(1, "{ x*Sin[x^2] ; x^4 }                      <0.0371612> // Stewart, Page 352 #35; Implied Domain;");
        runProblemArea(1, "{ x^4 ; 3x-x^3 }                          <1.1464674> // Stewart, Page 352 #36; Implied Domain;");
        runProblemArea(1, "{ 3x^2-2x ; x^3-3x+4 }                    <8.3780257> // Stewart, Page 352 #37; Implied Domain;");
        runProblemArea(1, "{ x*Cos[x] ; x^10 }                       <0.3028864> // Stewart, Page 352 #38; Implied Domain;");
