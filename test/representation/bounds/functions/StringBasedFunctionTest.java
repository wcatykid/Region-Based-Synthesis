...
//        testAll(1);
    ...
    private void testEvaluateAtPointByY(int indent)
    {
        ...
        for (int y = 0; y < 20; y++)
        {
            Assertions.Assert(f.evaluateAtPointByY(y*y), y);
        }
    ...
    private void testEvaluateAtPoint(int indent)
    {
        ...
        for (int x = -10; x < 10; x++)
        {
            Assertions.Assert(f.evaluateAtPoint(x), 2 * x * (1 - x));
        }
        ...
        for (int x = -10; x < 10; x++)
        {
            Assertions.Assert(trig.evaluateAtPoint(x), 0);
        }
    ...
    private void testTranslationHorizontally(int indent)
    {
        ...
        for (int x = -10; x < 10; x++)
        {
            Assertions.Assert(f.evaluateAtPoint(x), 2 * (x - h) * (1 - (x - h)));
        }
    ...
    private void testTranslationVertically(int indent)
    {
        ...
        for (int x = -10; x < 10; x++)
        {
            Assertions.Assert(f.evaluateAtPoint(x), 2 * (x - h) * (1 - (x - h)) + k);
        }
    ...
    private void testTranslationCombined(int indent)
    {
        ...
        for (int k = -100; k < 50; k++)
        {
            for (int h = -10; h < 10; h++)
            {
                ...
                for (int x = -10; x < 10; x++)
                {
                    Assertions.Assert(f.evaluateAtPoint(x), (x - h) * (x - h) * (1 - (x - h)) + k);
                }
            }
        }
    ...
    private void testStretchVertically(int indent)
    {
        ...
        for (int a = -50; a < 50; a++)
        {
            ...
            for (int x = -30; x < 30; x++)
            {
                Assertions.Assert(f.evaluateAtPoint(x), a * (x - h) * (x - h) * (1 - (x - h)) + k);
            }
    ...
    private void testStretchHorizontally(int indent)
    {
        ...
        for (int b = -50; b < 50; b++)
        {
            ...
            for (int x = -30; x < 30; x++)
            {
                Assertions.Assert(f.evaluateAtPoint(x), (b * (x - h)) * (b * (x - h)) * (1 - (b * (x - h))) + k);
            }
    ...
    private void testStretchCombined(int indent)
    {
        ...
        for (int a = -30; a < 50; a++)
        {
            for (int b = -30; b < 50; b++)
            {
                ...
                for (int x = -10; x < 10; x++)
                {
                    Assertions.Assert(f.evaluateAtPoint(x), a * (b * (x - h)) * (b * (x - h)) * (1 - (b * (x - h))) + k);
                }
        ...
        for (int a = -10; a < 10; a++)
        {
            for (int b = -10; b < 10; b++)
            {
                if (a != 0 && b != 0)
                {
                    ...
                    for (int x = -30; x < 30; x++)
                    {
                        Assertions.Assert(f.evaluateAtPoint(x), (1.0 / a) * ((1.0/b) * (x - h)) * ((1.0/b) * (x - h)) + k);
                    }
                }
            }
        }
